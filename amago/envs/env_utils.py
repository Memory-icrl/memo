...
class SequenceWrapper(gym.Wrapper):
    def __init__(
        ...
        if make_dset:
            ...
            # makedirs will fail in DDP since the directory is created on all ranks
            # instead we will create the directory in the main process by fetching the rank
            if not os.path.exists(self.dset_write_dir) and env_idx == 0 and is_main_process:
                print("env idx 0 making dir")
    ...
    def step(self, action):
        ...
        if timestep.terminal:
            # print(f"Total Return: {self.total_return} for env {self.env.env_name} with idx {self.env_idx}")
