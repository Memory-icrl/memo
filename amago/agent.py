...
from amago.nets import new_aux_future_predict
# from new_aux_future_predict import FuturePredict
...
class Multigammas:
    def __init__(
        self,
        # fmt: off
        discrete = [.7, .9, .95, .97, .99, .995], # .7 instead of .1
        continuous = [.7, .9, .95, .97, .99, .995],
        # fmt: on
    ):
...
class Agent(nn.Module):
    def __init__(
        ...
        use_aux_future_pred_loss: bool = False,
        ...
        self.traj_encoder = traj_encoder_Cls(
            ...
            horizon=horizon,
            tformer_type=tformer_type,
            summary_length=summary_length,
            segment_length=segment_length,
            detach_prob=detach_prob,
        )
        ...
        self.use_aux_future_pred_loss = use_aux_future_pred_loss
        self.aux_future_pred_loss_coeff = aux_future_pred_loss_coeff
        if use_aux_future_pred_loss:
            if tformer_type == "tformer":
                self.future_predict = new_aux_future_predict.OSFuturePredict(  # FuturePredict
                    self.emb_dim, action_space
                )
            elif tformer_type == "autocomp_tformer" or tformer_type == "rmt_tformer":
                self.future_predict = new_aux_future_predict.FuturePredictSegmented(    # do for this one later
                    self.emb_dim, action_space, segment_length
                )
    ...
    def forward(self, batch: Batch, log_step: bool):
        ...
        critic_mask = repeat(state_mask[:, 1:, ...], f"b l 1 -> b l {C} {G} 1").bool()
        ...
        if not self.fake_filter or self.online_coeff > 0:
            ...
            with torch.no_grad():
                ...
                assert q_targ_sp_ap_gp.shape == (B, L - 1, C, G, 1)
                # if gammas is not on gpu, move it to the device
                if self.gammas.device != r.device:
                    self.gammas = self.gammas.to(r.device)
                gamma = self.gammas.unsqueeze(-1)
        ...
        ######################
        if self.use_aux_future_pred_loss:
            aux_loss, image_list = self.future_predict.calc_loss(
                batch,
                s_rep,
            )
            actor_loss += self.aux_future_pred_loss_coeff * aux_loss
            if log_step and torch.distributed.get_rank() == 0:   
                # we want to log the image pairs in wandb through the update_info
                # but image_list is a list of len 1, and it contains a tuple of two numpy float arrays in the range 0-1 consisting of 10 images - the ground truth and predicted images

                # image_list[0][0] and image_list[0][1] are float tensors of shape (10, 64, 64, 3), make them grids of 10 images using make_grid
                # print(image_list[0][0].shape)
                gt_img_grid = torchvision.utils.make_grid(torch.tensor(image_list[0][0]), nrow=5) 
                pred_img_grid = torchvision.utils.make_grid(torch.tensor(image_list[0][1]), nrow=5)

                # also save the grids locally
                # torchvision.utils.save_image(gt_img_grid, "gt_image.png")
                # torchvision.utils.save_image(pred_img_grid, "pred_image.png")
                # print(gt_img_grid.shape)

                self.update_info.update({
                    "aux_loss": aux_loss,
                    "gt_image": wandb.Image(gt_img_grid),
                    "pred_image": wandb.Image(pred_img_grid)
                })
