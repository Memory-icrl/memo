...
import ipdb
import amago.envs.builtin.mmaze as mmaze
from matplotlib import pyplot as plt

def get_maze_obs_space(keepkeys=['image', 'prev_action']):
    all_obs_space = {
        'image': gym.spaces.Box(0, 255, (64, 64, 3), dtype=np.float32),
        'prev_action': gym.spaces.Box(-np.inf, np.inf, (1,), dtype=np.float32),
        'agent_dir': gym.spaces.Box(-np.inf, np.inf, (2,), np.float32),
        'agent_pos': gym.spaces.Box(-np.inf, np.inf, (2,), np.float32),   
        ...
        'target_coords': gym.spaces.Box(-np.inf, np.inf, (10, 2), np.float32), 
    ...
    obs_space = gym.spaces.Dict({k: all_obs_space[k] for k in keepkeys})
    return obs_space
...
class Experiment:
    ...
    def init_envs(self):
        ...
        make_train = [
            ...
            for env_idx, env_func in enumerate(make_train_envs)
        ]
        make_val = [
            EnvCreator(
                make_env=env_func,
            ...
            for env_idx, env_func in enumerate(make_val_envs)
        ]

        # make parallel envs
        Par = gym.vector.AsyncVectorEnv if self.async_envs else DummyAsyncVectorEnv
        self.train_envs = Par(make_train, observation_space=get_maze_obs_space(self.keepkeys), action_space=gym.spaces.Discrete(6)) if (self.use_maze and self.async_envs) else Par(make_train)
        self.train_envs.reset()
        self.val_envs = Par(make_val, observation_space=get_maze_obs_space(self.keepkeys), action_space=gym.spaces.Discrete(6)) if (self.use_maze and self.async_envs) else Par(make_val)
        self.val_envs.reset()
        ...
        if self.use_maze and self.async_envs:
            assert self.env_size>0, "env_size must be set to 9, 11, 13 or 15"
            horizons = {9: 1000, 11: 2000, 13: 3000, 15: 4000}
            self.horizon = horizons[self.env_size]
            self.gcrl2_space = gym.spaces.Dict(
                {
                    "obs": get_maze_obs_space(self.keepkeys),
                    "goal": gym.spaces.Box(low=0.0, high=0.0, shape=(1, 1)), 
                    "rl2": gym.spaces.Box(
                        shape=(9,), # action + reward + done + time
                        dtype=np.float32,
                        low=float("-inf"),
                        high=float("inf"),
                    ),
    ...
    def init_checkpoints(self):
        ...
        self.img_dir = os.path.join(self.dset_root, self.dset_name, "imgs")

        # only main process should create the directory
        ...
        #     raise ValueError(
        #         f"Checkpoint directory {self.ckpt_dir} already exists. "
        #         "Please remove it before starting a new experiment."
        #     )
    ...
    def init_dloaders(self):
        ...
        # make a distributed sampler so that each process gets a unique subset of the data
        # (this is necessary because the data is stored in a single directory)
        # print(self.accelerator.process_index)
        # print(self.accelerator.num_processes)
        # sampler = torch.utils.data.distributed.DistributedSampler(
        #     self.train_dset, num_replicas=self.accelerator.num_processes, rank=self.accelerator.process_index,
        # )
        print("dloader workers: ", self.dloader_workers)
    ...
    def init_model(self):
        policy_kwargs = {
            ...
            "tformer_type": self.tformer_type,
            "summary_length": self.summary_length,
            "segment_length": self.segment_length,
            "detach_prob": self.detach_prob,
            "use_aux_future_pred_loss": self.use_aux_future_pred,
            "aux_future_pred_loss_coeff": self.aux_future_pred_loss_coeff,
        }
    ...
    def interact(
        ...
        if hidden_state is None:
            ...
            if self.tformer_type == "autocomp_tformer" or self.tformer_type == "rmt_tformer":
                # print(policy.traj_encoder.tformer.interaction_step)
                policy.traj_encoder.tformer.interaction_step = 0
        ...
        for _ in iter_:
            ...
            _, rewards, terminated, truncated, _ = envs.step(actions)
            traj_rew.append(rewards)
    ...
    def evaluate_val(self, return_metrics: bool = False, plot_traj_rew: bool = False):
        if self.val_timesteps_per_epoch > 0:
            *_, returns, successes = self.interact(
                self.val_envs,
                self.val_timesteps_per_epoch,
                plot_traj_rew=plot_traj_rew,
            )
            ...
            return returns, successes, logs_global
    ...
    def evaluate_test(
        ...
        Par = gym.vector.AsyncVectorEnv if self.async_envs else DummyAsyncVectorEnv
        if self.use_maze and self.async_envs:
            test_envs = Par([make for _ in range(self.parallel_actors)], observation_space=get_maze_obs_space(self.keepkeys), action_space=gym.spaces.Discrete(6))
        else:
        ...
        for ret, suc in zip(returns, successes):
            for env_name, scores in ret.data.items():
                if env_name in return_by_env_name:
                    return_by_env_name[env_name] += scores.copy()
                else:
                    return_by_env_name[env_name] = scores.copy()
    ...
    def learn(self):
        ...
        for epoch in range(start_epoch, self.epochs):
            if self.always_load_latest and epoch == start_epoch:
                self.read_latest_policy()

            # environment interaction
            self.policy_aclr.eval()
            if self.val_interval and epoch % self.val_interval == 0 and epoch != 0:
                self.evaluate_val()
