...
class TformerTrajEncoder(TrajEncoder):
    def __init__(
        ...
        elif tformer_type == "autocomp_tformer":
            self.tformer = transformer.AutoCompressorTransformer(
                inp_dim=tstep_dim,
                max_pos_idx=horizon,
                d_model=d_model,
                d_ff=d_ff,
        ...
        elif tformer_type == "rmt_tformer":
            self.tformer = transformer.RmtTransformer(
                inp_dim=tstep_dim,
                max_pos_idx=horizon,
                d_model=d_model,
                d_ff=d_ff,
    ...
    def init_hidden_state(self, batch_size: int, device: torch.device):
        def make_cache():
            return transformer.Cache(
                ...
                max_seq_len=self.max_seq_len + (2*self.summary_length) + 1 ,
